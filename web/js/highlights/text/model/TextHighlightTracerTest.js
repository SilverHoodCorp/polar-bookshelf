var assert = require('assert');

const {TextHighlightTracer} = require("./TestHighlightTracer");
const {TextHighlight} = require("../../../metadata/TextHighlight");
const {DocMeta} = require("../../../metadata/DocMeta");
const {DocMetas} = require("../../../metadata/DocMetas");
const {TextHighlightRecords} = require("../../../metadata/TextHighlightRecords");
const {assertJSON} = require("../../../test/Assertions");
const {TextHighlightView} = require("../TextHighlightView");
const {Proxies} = require("../../../proxies/Proxies");

require("../../../test/TestingTime").freeze();

describe('TextHighlightTracer', function() {

    // https://stackoverflow.com/questions/8024149/is-it-possible-to-get-the-non-enumerable-inherited-property-names-of-an-object
    //
    // FIXME: teh bug is that : getOwnProperty returns addTraceListener..
    // I want to hide this to maybe add it to the Object prototype...

    describe('Listen for new highlights', function() {

        it("Initial values", function () {

            let docMeta = createDocMeta();

            let textHighlightTracer = new TextHighlightTracer();

            let myTextHighlightListener = new MyTextHighlightListener();

            textHighlightTracer.register(docMeta, myTextHighlightListener );

            let expected = [
                {
                    "pageNum": 1,
                    "textHighlight": {
                        "created": "2012-03-02T11:38:49.321Z",
                        "lastUpdated": "2012-03-02T11:38:49.321Z",
                        "rects": {
                            "0": {
                                "top": 100,
                                "left": 100,
                                "right": 200,
                                "bottom": 200,
                                "width": 100,
                                "height": 100
                            }
                        },
                        "textSelections": {
                            "0": "hello world"
                        },
                        "text": "hello world",
                        "thumbnail": null
                    },
                    "mutationType": "INITIAL"
                }
            ];

            assertJSON(myTextHighlightListener.mutations, expected);

        });

        it("New text highlights on new pages", function () {

            let docMeta = createDocMeta();

            let textHighlightTracer = new TextHighlightTracer();

            let myTextHighlightListener = new MyTextHighlightListener();

            textHighlightTracer.register(docMeta, myTextHighlightListener );

            myTextHighlightListener.mutations = [];

            let textHighlightRecord = createTextHighlightRecord();

            docMeta.getPageMeta(3).textHighlights[textHighlightRecord.id] = textHighlightRecord.value;

            let expected = [
                {
                    "pageNum": 3,
                    "textHighlight": {
                        "created": "2012-03-02T11:38:49.321Z",
                        "lastUpdated": "2012-03-02T11:38:49.321Z",
                        "rects": {
                            "0": {
                                "top": 100,
                                "left": 100,
                                "right": 200,
                                "bottom": 200,
                                "width": 100,
                                "height": 100
                            }
                        },
                        "textSelections": {
                            "0": "hello world"
                        },
                        "text": "hello world",
                        "thumbnail": null
                    },
                    "mutationType": "SET"
                }
            ];

            assertJSON(myTextHighlightListener.mutations, expected);

        });

    });

});

class MyTextHighlightListener {

    constructor() {
        this.mutations = [];
    }


    onTextHighlight(textHighlightEvent) {
        this.mutations.push({pageNum: textHighlightEvent.pageMeta.pageInfo.num, textHighlight: textHighlightEvent.textHighlight, mutationType: textHighlightEvent.mutationType});
    }

    onTextHighlightDeleted(textHighlightEvent) {
        this.mutations.push({pageNum: textHighlightEvent.pageMeta.pageInfo.num, textHighlight: textHighlightEvent.textHighlight, mutationType: textHighlightEvent.mutationType});
    }

};

function createDocMeta() {

    let fingerprint = "110dd61fd57444010b1ab5ff38782f0f";

    let docMeta = DocMetas.createWithinInitialPagemarks(fingerprint, 14);
    DocMetas.addPagemarks(docMeta, {nrPages: 1, offsetPage: 4, percentage: 50})

    // create some initial highlights.

    let textHighlightRecord = createTextHighlightRecord();

    docMeta.getPageMeta(1).textHighlights[textHighlightRecord.id] = textHighlightRecord.value;

    return Proxies.create(docMeta).deepTrace();

}

function createTextHighlightRecord() {

    let rects = [ {top: 100, left: 100, right: 200, bottom: 200, width: 100, height: 100}];
    let textSelections = ["hello world"];
    let text = "hello world";

    return TextHighlightRecords.create(rects, textSelections, text);

}
